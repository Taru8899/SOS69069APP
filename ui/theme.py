...
def get_app_version() -> str:
    """User-facing version (display number only, no v prefix)."""
    return APP_DISPLAY_VERSION


def format_app_version() -> str:
    """Display form with v clearly separated: 'v 1.6.9.0.6.9'."""
...
def PageScroll(**kwargs):
    """Full-page middle scroller: same as ID page, no white side bar."""
    ...
    try:
        kwargs.setdefault("scroll_type", ["content"])
    except Exception:
        pass
...
class PayerBar(BoxLayout):
    ...
    def refresh(self):
        ...
        if payer:
            try:
                full = address_from_private_key(payer)
                self.addr_lbl.text = "Gas payer: " + full + " ↗"
                self.addr_lbl.url = etherscan_address(full)
            except Exception:
                self.addr_lbl.text = "Gas payer: (error)"
                self.addr_lbl.url = None
        else:
